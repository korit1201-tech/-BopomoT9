...
- ✍️ **雙引擎高精度手寫模式（Google ML Kit + 本地備用）**：
  - 空白鍵左右滑動自由輪替「繁」→「簡」→「手」。
  - **Google 官方 ML Kit Digital Ink Recognition**（與 Gboard 同款端側 AI 辨識模型）：支援繁體中文（`zh-Hant`），完美辨識連筆、草書、任意筆順及中英數混寫，識別準確率高達 99%。
  - **8,000+ 繁體字本地筆順雙保險引擎**：若無網路或模型尚未下載，無縫切換至本地五筆幾何特徵引擎（橫豎撇捺折），保障離線輸入體驗。
