...
- ✍️ **Google ML Kit 高精度手寫輸入**：
  - 空白鍵左右滑動自由輪替「繁」→「簡」→「手」。
  - 採用 **Google 官方 ML Kit Digital Ink Recognition**（與 Gboard 同款端側 AI 神經網路辨識模型）。
  - 支援繁體中文（`zh-Hant`），完美支援草書、行書連筆、筆劃順序無拘束及中英數標點混寫，識別率精準靈敏。
  - 模型下載至本地後即在端側（On-Device）離線推論，兼顧高準確率與隱私安全。
- 📏 **鍵盤高度自由拉伸縮放（Drag-to-Resize）**：
  - 候選字列與主鍵盤間提供精緻調整把手。
  - 隨時上下拖曳自由縮放鍵盤高度（180dp ~ 380dp），鍵盤按鍵與字體等比例自適應放大，大螢幕與大字體打字更輕鬆。
  - 自動記憶調整後的高度設定，重啟後依然保持自訂尺寸。
- ⌨️ **PC 級實體鍵盤支援**：
...
- 🔢 **純數字模式最佳化（計算機直覺符號）**：
  - 切換至數字模式時，**左側 5 格符號列自動轉變為 `+` `-` `*` `/` `=` 運算符號**，點擊直出、長按可選關聯符號（`±`、`—`、`×`、`÷`、`≠`、`≤`、`≥`）。
  - 中央 12 鍵數字盤底部由左至右分別為 **小數點 `.`、數字 `0`、井號 `#`**，算術與輸入金額更加順暢。
- 🔠 **多元英文輸入模式**：
  - **英文 9 鍵（T9 Multi-tap & 長按快選）**：支援連按輪替字母（A → B → C）、滑動直出，以及**長按任意按鍵直接彈出字母/數字選單**直出。
  - **英文 26 鍵（QWERTY 整合常用運算與括號符號）**：
    - 頂部直接整合**常用符號專屬列（`+` `-` `*` `/` `=` `(` `)` `@` `_` `&`）**，點擊一鍵直出。
    - 符號鍵長按呼出完整家族：如 `(` 長按選 `[` `{` `<`，`)` 長按選 `]` `}` `>`，`=` 長按選 `≠` `≈` `≤` `≥`。
    - 字母鍵長按快捷輸入：第一列長按直出數字（`1` ~ `0`），二三列長按直出問號驚嘆號與標點。
    - 支援 Shift 大小寫鎖定與專屬連退鍵。
- 📳 **系統級觸覺震動回饋**：
  - 自動偵測 Android 系統震動開關，開關跟隨手機系統設定。
- 🔒 **端側運算 & 隱私安全**：
  - 零第三方統計 SDK、零按鍵記錄，手寫模型下載後即在端側離線推論，全面保護隱私。
